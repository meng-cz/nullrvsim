...
public:
    SimpleTickQueue(uint32_t input_wid, uint32_t output_wid, uint32_t queue_size) {
        ...
        owid = output_wid;
        qsize = queue_size;
        len = ALIGN((iwid + owid + qsize) * 2, 16);
        bottom = q_bottom = push_bottom = ptr = pop_sz = q_sz = push_sz = 0;
        buf.resize(len);
    ...
    virtual void apply_next_tick() {
        if(qsize) {
            uint32_t q2pop = std::min<uint32_t>(owid - pop_sz, q_sz);
    ...
    inline uint32_t can_push() {return iwid - push_sz;}
    inline uint32_t can_pop() {return pop_sz;}

    inline T& top() {
        if(pop_sz == 0) [[unlikely]] assert(0);
        return buf[bottom];
    }
    ...
    inline bool push(T &v) {
        if(push_sz >= iwid) [[unlikely]] return false;
        buf[ptr] = v;
        ptr = (ptr + 1) % len;
        push_sz++;
        return true;
    ...
    inline void clear(std::list<T> *out = nullptr) {
        if(out) {
            for(uint32_t i = bottom ; i != ptr; i = (i+1)%len) {
                out->push_back(buf[i]);
            }
        }
        bottom = q_bottom = push_bottom = ptr = pop_sz = q_sz = push_sz = 0;
...
protected:
    ...
    uint32_t len, bottom, q_bottom, push_bottom, ptr;
    uint32_t pop_sz, q_sz, push_sz;
    std::vector<T> buf;
};
...
//     ~SimpleTickQueue() {}
//     virtual void apply_next_tick() {
//         if(qsize) while(pop_buf.size() < owid && !q.empty()) pop_buf.splice(pop_buf.end(), q, q.begin());
//         while(pop_buf.size() < owid && !push_buf.empty()) pop_buf.splice(pop_buf.end(), push_buf, push_buf.begin());
//         if(qsize) while(q.size() < qsize && !push_buf.empty()) q.splice(q.end(), push_buf, push_buf.begin());
//     }
